const ptTranslations = {
  translation: {
    home: "Início",
    about: "Sobre",
    contact: "Contato",
    welcome: "Bem-vindo ao nosso portal de notícias!",
    change_language: "Trocar idioma",
    "The Wall Street Journal": "O Jornal de Wall Street",
    "Read News": "Ler Notícia",
    "Read more": "Leia mais",
    News: "Notícia",
    Author: "Autor(a)",
    "John Gruber": "John Gruber",
    "Joanna Stern: ‘Google’s Gemini Live AI Sounds So Human, I Almost Forgot It Was a Bot’":
      "Joanna Stern: ‘O Gemini Live AI do Google parece tão humano que quase esqueci que era um bot’",
    "Anne Tergesen and Oyin Adedoyin": "Anne Tergesen e Oyin Adedoyin",
    "They are the default choice for millions of retirement savers who are automatically enrolled in 401(k) plans. But some investors are moving out of them.":
      "Eles são a escolha padrão para milhões de poupadores de aposentadoria que estão automaticamente inscritos em planos 401(k). Mas alguns investidores estão abandonando-os.",
    "The 401(k) Investors Convinced That Target-Date Funds Miss the Mark":
      "Os investidores 401 (k) estão convencidos de que os fundos para datas-alvo erram o alvo",
    "Emily Bary, Therese Poletti, Tomi Kilgore":
      "Emily Bary, Therese Poletti, Tomi Kilgore",
    "Apple reported results for its fiscal third quarter after the close of trading on Thursday. Follow along live as MarketWatch analyzes the expectations and results.":
      "A Apple divulgou os resultados do terceiro trimestre fiscal após o fechamento do pregão na quinta-feira. Acompanhe ao vivo enquanto o MarketWatch analisa as expectativas e os resultados.",
    "Tomi Kilgore": "Tomi Kilgore",
    "Apple earnings: Stock rises as iPhone revenue beats estimates":
      "Lucros da Apple: as ações sobem à medida que a receita do iPhone supera as estimativas",
    "Amazon plans to report second-quarter earnings after the close of trading Thursday. MarketWatch will be breaking down the expectations and results.":
      "A Amazon planeja divulgar os lucros do segundo trimestre após o fechamento das negociações na quinta-feira. MarketWatch irá detalhar as expectativas e resultados.",
    "Amazon made $400 million on its equity investments in the second quarter":
      "Amazon faturou US$ 400 milhões em seus investimentos de capital no segundo trimestre",
    "Joseph Adinolfi, Emily Bary, Vivien Lou Chen, Isabel Wang, Ciara Linnane, Jamie Chisholm":
      "Joseph Adinolfi, Emily Bary, Vivien Lou Chen, Isabel Wang, Ciara Linnane, Jamie Chisholm",
    "Follow all the latest U.S. market action for Thursday here.":
      "Acompanhe todas as últimas ações do mercado dos EUA na quinta-feira aqui.",
    "U.S. stocks forfeit early gains as Wall Street awaits Amazon, Apple earnings":
      "Ações dos EUA perdem ganhos iniciais enquanto Wall Street aguarda ganhos da Amazon e da Apple",
    "Steve Goldstein, Jamie Chisholm": "Steve Goldstein, Jamie Chisholm",
    "Follow live coverage of the Bank of England rate decision.":
      "Acompanhe a cobertura ao vivo da decisão da taxa do Banco da Inglaterra.",
    "Bank of England cuts rates for first time in over four years":
      "Banco da Inglaterra reduz taxas pela primeira vez em mais de quatro anos",
    "Emily Bary, Tomi Kilgore": "Emily Bary, Tomi Kilgore",
    "Meta reported its second-quarter earnings after Wednesday's close. MarketWatch will be breaking down the expectations and results.":
      "A Meta divulgou seus lucros do segundo trimestre após o fechamento de quarta-feira. MarketWatch irá detalhar as expectativas e resultados.",
    "Meta earnings: Stock rises as the Facebook parent gives upbeat outlook":
      "Meta ganhos: as ações sobem à medida que a controladora do Facebook apresenta perspectivas otimistas",
    "Jeffry Bartash, Greg Robb, Steve Goldstein":
      "Jeffry Bartash, Greg Robb, Steve Goldstein",
    "Follow all the action from the Federal Open Market Committee meeting and Jerome Powell press conference.":
      "Acompanhe toda a ação da reunião do Comitê Federal de Mercado Aberto e da coletiva de imprensa de Jerome Powell.",
    "WATCH: Jerome Powell speaks after the Fed's interest-rate decision":
      "ASSISTA: Jerome Powell fala após a decisão do Fed sobre a taxa de juros",
    "Isabel Wang": "Isabel Wang",
    "Follow all the latest U.S. market action for Wednesday here.":
      "Acompanhe todas as últimas ações do mercado dos EUA na quarta-feira aqui.",
    "Why the Fed has to 'thread a needle' today, according to this strategist":
      "Por que o Fed tem que “enfiar a linha na agulha” hoje, de acordo com este estrategista",
    "Therese Poletti, Emily Bary, Tomi Kilgore":
      "Therese Poletti, Emily Bary, Tomi Kilgore",
    "AMD reported second-quarter results after Tuesday's closing bell, and investors will be keenly focused on the company's AI revenue forecast. Follow along live as MarketWatch breaks down the results.":
      "A AMD divulgou os resultados do segundo trimestre após o fechamento de terça-feira, e os investidores estarão profundamente focados na previsão de receita de IA da empresa. Acompanhe ao vivo enquanto o MarketWatch detalha os resultados.",
    "AMD sees gross margins climb amid record data-center results":
      "AMD vê margens brutas subirem em meio a resultados recordes de data center",
    "Heather Haddon": "Heather Haddon",
    "The fast-food giant said it is focusing on value with consumers continuing to tighten their budgets.":
      "A gigante do fast-food disse que está se concentrando no valor, com os consumidores continuando a restringir seus orçamentos.",
    "McDonald’s Sales Soften, Sounding Alarm for Restaurants as Diners Curb Visits":
      "As vendas do McDonald's diminuem, soando o alarme para os restaurantes enquanto os clientes restringem as visitas",
    "Alexander Osipovich": "Alexander Osipovich",
    "Tuttle helped persuade companies such as Palantir and Snowflake to list on the Big Board, rather than Nasdaq.":
      "Tuttle ajudou a persuadir empresas como Palantir e Snowflake a listar-se no Big Board, em vez de Nasdaq.",
    "NYSE’s Longtime IPO Pitchman, John Tuttle, to Leave After 17 Years":
      "O lançador de IPO de longa data da NYSE, John Tuttle, sairá após 17 anos",
    "About The Wall Street Journal": "Sobre o Wall Street Journal",
    "Bringing you the latest news, insights, and analysis from around the world.":
      "Trazendo a você as últimas notícias, insights e análises de todo o mundo.",
    "Our Mission": "Nossa Missão",
    "At The Wall Street Journal, our mission is to deliver accurate, timely, and in-depth news coverage. We strive to be a trusted source of information for our readers, providing them with the insights they need to make informed decisions.":
      "No The Wall Street Journal, nossa missão é fornecer cobertura noticiosa precisa, oportuna e aprofundada. Nós nos esforçamos para ser uma fonte confiável de informações para nossos leitores, fornecendo-lhes os insights de que precisam para tomar decisões informadas.",
    "Our Vision": "Nossa Visão",
    "Our vision is to be the leading news outlet globally, known for our commitment to integrity, quality journalism, and comprehensive reporting. We aim to shape the future of news by embracing innovation and adapting to the changing needs of our audience.":
      "Nossa visão é ser o principal veículo de notícias do mundo, conhecido por nosso compromisso com a integridade, jornalismo de qualidade e reportagens abrangentes. Nosso objetivo é moldar o futuro das notícias, abraçando a inovação e adaptando-nos às novas necessidades do nosso público.",
    "Our History": "Nossa História",
    "The Wall Street Journal was founded with the goal of providing in-depth and reliable news to readers worldwide. Over the years, we have grown into a leading news organization, recognized for our thorough reporting, expert analysis, and balanced perspectives. Our history is rooted in a tradition of excellence, and we continue to build on that legacy today.":
      "O Wall Street Journal foi fundado com o objetivo de fornecer notícias detalhadas e confiáveis ​​aos leitores de todo o mundo. Ao longo dos anos, nos tornamos uma organização de notícias líder, reconhecida por nossas reportagens completas, análises especializadas e perspectivas equilibradas. Nossa história está enraizada em uma tradição de excelência e continuamos a desenvolver esse legado até hoje.",
    "Meet Our Team": "Conheça nossa equipe",
    "Tom Welling": "Tom Welling",
    "Chloe Sullivan": "Chloe Sullivan",
    "Michael Rosen": "Michael Rosen",
    "Editor in Chief": "Editor-chefe",
    "Senior Reporter": "Repórter Sênior",
    "Lead Designer": "Designer Líder",
    "Our Core Values": "Nossos Valores Fundamentais",
    "Integrity, excellence, and responsibility are at the heart of everything we do at The Wall Street Journal. These values guide our work and ensure that we provide our readers with the highest standard of journalism.":
      "Integridade, excelência e responsabilidade estão no centro de tudo o que fazemos no The Wall Street Journal. Esses valores norteiam nosso trabalho e garantem que oferecemos aos nossos leitores o mais alto padrão de jornalismo.",
    "We believe in the power of truth and the importance of delivering news that is not only accurate but also fair and balanced. Our commitment to these principles is unwavering, and it is what sets us apart in the world of journalism.":
      "Acreditamos no poder da verdade e na importância de transmitir notícias que não sejam apenas precisas, mas também justas e equilibradas. O nosso compromisso com estes princípios é inabalável e é o que nos diferencia no mundo do jornalismo.",
    "Contact Us": "Contate-nos",
    "We would love to hear from you! If you have any questions, comments, or feedback, please don't hesitate to reach out to us at contact@wallstreetjournal.com.":
      "Adoraríamos ouvir de você! Se você tiver alguma dúvida, comentário ou feedback, não hesite em entrar em contato conosco pelo e-mail contact@wallstreetjournal.com.",
    "We are here to help! Get in touch with us using the form below or through our contact information.":
      "Estamos aqui para ajudar! Entre em contato conosco através do formulário abaixo ou através de nossos dados de contato.",
    "Contact Information": "Informações de contato",
    "E-mail": "E-mail",
    Phone: "Telefone",
    WhatsApp: "WhatsApp",
    Address: "Endereço",
    Name: "Nome",
    "Contact Form": "Formulário de contato",
    "Follow Us": "Siga-nos",
    "Our Location": "Nossa localização",
    Subject: "Assunto",
    Message: "Mensagem",
    Send: "Enviar",
    "Developed by": "Desenvolvido por",
    "Copyright All Rights Reserved":
      "Direitos autorais Todos os direitos reservados",
    "We would love to hear from you! If you have any questions, comments, or feedback, please don't hesitate to reach out to us at":
      "Adoraríamos ouvir de você! Se você tiver alguma dúvida, comentário ou feedback, não hesite em entrar em contato conosco em",
  },
};

export default ptTranslations;
